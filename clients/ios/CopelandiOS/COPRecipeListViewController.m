...
#define kNodeListJson @"http://copelandia.dev/node.json"
      ...
      for (id ingredient in [node valueForKey:@"field_recipe_ingredients"]) {
        ...
        if (responseData) {
          ...
          if (ingredientError) {
            ...
            // See if the Ingredient already exists on the Recipe
            if ([newRecipe.ingredients count] > 0) {
              for (Ingredient *ing in newRecipe.ingredients) {
                if ([ing.itemID isEqualToString:ingredientItemID]) {
                  ingredientMO = ing;
                  break;
                }
              }
            ...
            // If the Ingredient was not found on the Recipe, check Core Data for it
            if (!ingredientMO) {
